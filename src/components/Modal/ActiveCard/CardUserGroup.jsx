...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  // Lấy thông tin activeBoard hiện tại để lấy thông tin user thông qua FE_allUsers
  const board = useSelector(selectCurrentActiveBoard)
  /**
   * Thành viên trong card phải là tập con của board, nếu không thì sẽ không hiển thị được
   */
  ...
  const handleUpdateCardMembers = (user) => {
    // Create incomingMemberInfo to send to Backend with 2 properties: userId & action to add or remove
    const incomingMemberInfo = {
      userId: user._id,
      action: cardMemberIds.includes(user._id) ? CARD_MEMBER_ACTIONS.REMOVE : CARD_MEMBER_ACTIONS.ADD
    }

    // Call onUpdateCardMembers to update card members
